...
for (let i = 0; i < acc.length; i++) {
  ...
  .then(() => {
    ...
    console.log(`${a[1]} - Logged in. Doing stuff in ${timeOut/1000} seconds`);
    setTimeout(() => {
      new Promise((resolve) => {
...
function pickTrainerName (client, a) {
  a[5] = a[5].replace(/\r/g, '');
  if (a[5].length > 12) {
    console.log(`${a[1]} - Seems like ${a[5]} is too long name... (PROTIP: It can't exceed 12 characters!) Run this to try different name!:
node name.js -a ${a[0]} -u ${a[1]} -p ${a[2]} -l ${a[3]},${a[4]} -u TRAINER-NAME`);
  ...
  .then(data=>{
    if (data.is_assignable) {
      ...
      console.log(`${a[1]} - Seems like there's already someone known as ${a[5]}... Run this to try different name!:
node name.js -a ${a[0]} -u ${a[1]} -p ${a[2]} -l ${a[3]},${a[4]} -u TRAINER-NAME`);
    }
